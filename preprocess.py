...
def get_transform(name='imagenet', input_size=None,
    ...
    if name == 'imagenet':
        ...
        input_size = 32  # CIFAR-10 input size
        normalize = {
        'mean': [0.4914, 0.4822, 0.4465],
        'std': [0.2023, 0.1994, 0.2010]
       }
        if augment:
            scale_size = 40  # Resize for augmentation
            return pad_random_crop(input_size, scale_size=scale_size, normalize=normalize)
        else:
            return scale_crop(input_size=input_size, scale_size=input_size, normalize=normalize)
