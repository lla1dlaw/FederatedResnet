...
class ResultsLog(object):
    ...
    def add(self, **kwargs):
        ...
        if self.results is None:
            ...
            self.results = pd.concat([self.results,df], ignore_index=True) # Hossein
...
def save_checkpoint(state, is_best, clientid, path='.', filename='checkpoint_%d.pth.tar', save_all=False):
    filename = os.path.join(path, filename)
    torch.save(state, filename % clientid)
    if is_best:
        shutil.copyfile(filename % clientid, os.path.join(path, 'model_best_%d.pth.tar' % clientid))
    if save_all:
        shutil.copyfile(filename % clientid, os.path.join(
            path, 'checkpoint_epoch_%s_%d.pth.tar' % (state['epoch'], clientid)))
